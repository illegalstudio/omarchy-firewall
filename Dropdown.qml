...
// chrome; the popup anchors below and uses Color.popups.background plus
// Color.popups.border so it reads as a panel surface instead of the
...
Item {
  ...
  // own keyCatcher so j/k inside the popup do not double-drive the panel
  // cursor.
        ...
        contentItem: ListView {
          ...
          WheelHandler {
            ...
            onWheel: function(event) {
              var minimumY = optionList.originY !== undefined
                ? optionList.originY : 0
              var maximumY = minimumY + optionList.contentHeight
                - optionList.height

              if (maximumY <= minimumY) return

              optionList.contentY = Math.max(
                minimumY,
                Math.min(
                  optionList.contentY - event.angleDelta.y,
                  maximumY
                )
              )
